...
begin
	...
	if v_contador <= 240 then
		INSERT INTO pago_vivienda VALUES (:NEW.pago_vivienda_id, 
			:NEW.num_pago, :NEW.vivienda_id, :NEW.importe_pago, 
			:NEW.fecha_pago, :NEW.deposito_realizado_pdf);
	ELSE
		raise('Ya no se admiten registros');
