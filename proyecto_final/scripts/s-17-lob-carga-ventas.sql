...
set serveroutput on
Prompt Actualizando pdf de ventas
...
create or replace procedure p_actualiza_venta is
	v_bfile bfile;
...
cursor cur_venta is
	select vivienda_id,avaluo_pdf
	from vivienda_venta;

begin

	for r in cur_venta loop
		v_src_offset := 1;
		v_dest_offset := 1;
		...
		select avaluo_pdf into v_dest_blob
		from vivienda_venta
